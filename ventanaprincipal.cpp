...
void VentanaPrincipal::crearMenus(){
	...
	menuBolaImagenes = menuBar() ->addMenu("RestaurarColor");
...
void VentanaPrincipal::slotPosicionBolas(){
	...
	if(dPosicionBolas == NULL){
		...
		connect(dPosicionBolas,SIGNAL(senyalBolaImagen(Bola*)),this,SLOT(slotBolaImagen(Bola*)));
	}
	dPosicionBolas->show();
}
...
void VentanaPrincipal::slotEliminarImagen(){
	...
	/*QVariant dato = actCulp->data();
	void *puntero = dato.Variantvalue<void *>();*/
